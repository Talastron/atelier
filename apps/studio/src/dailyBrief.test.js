...
describe('daily-brief freshness history (recent clothing bases)', () => {
  ...
  it('readRecentBases returns [] when stored data is a non-array shaped like an entry', () => {
    localStorage.setItem('atelier.dailyBrief.recent.u1',
      JSON.stringify({ dateKey: '2026-07-16', baseIds: ['a'] }));
    expect(readRecentBases('u1')).toEqual([]);
  });
